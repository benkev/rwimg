...
#if defined(TBR) && defined(TBRIV)
#error ERROR: either TBR or TBRIV can be defined, not both
#endif

#if defined(TBR)
void advance_beam_Tbr(
#endif
#if  defined(TBRIV)
void advance_beam_TbrIV( 
#endif
#if !defined(TBR) && !defined(TBRIV)
void advance_beam_basic( 
#endif
	int nRay, 
        double Param_I[],
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
	...
	xi = 0.1148650403; // cm^6 K^1.5 Hz^2 -- from Smerd
      ...
      I_pr =  TbrIV_II[iRay][0];
      V_pr =  TbrIV_II[iRay][1];
      TbrIV_II[iRay][0] = I_pr - (I_pr + eta*V_pr - Te)*dOpDepth;
      TbrIV_II[iRay][1] = V_pr - (eta*I_pr + V_pr     )*dOpDepth;
      /* if (iRay == 1225){ */
