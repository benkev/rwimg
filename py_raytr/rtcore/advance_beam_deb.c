...
/*
 * "Debug" version of advance_beam.c, with printouts of the variables
 * for a selected ray, TrcRay.
 */
...
void advance_beam(int nRay, 
  ...
  /* double extern const cProtonChargeCGSe; //=4.8e-10;// StatCoulombs, CGSe */
  ...
  /* double extern const Te_corona; // = 1.0e6; // K */
  /* double extern const Te_chromo; // = 3.0e4; // K */
  /* double extern const R0; // = 6.950e5;   // km, Solar radius */
  /* double extern const h_chromo; // = 10000.0; // km, Chromosphere height */
  ...
  //int const static TrcRay = 10000000; // switch off
  ...
  for (iRay=0; iRay < nRay; iRay++) {
    ...
    //if (INACTIVE & Flags_I[iRay]) continue;   //------------------------->>
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
	...
	SolDist += pow(Pos_ID[iRay][i],2);
      ...
      printf("AFTER Get_Pl_Den: Rho_I = %g, RhoCr = %g, Flags_I = %i\n", 
	     Rho_I[iRay], prm->RhoCr, Flags_I[iRay]);
      printf("                : Eps = %g\n", Eps); 
	...
	if (DirPr_ID[iRay][2] < 0.0) { // if max count exceeded
	  /* Flags_I[iRay] |= (BADRAY|INACTIVE); // Do not process the ray */
	...
	if (Curv >=  prm->Tol2) { // Testing curvature 
	...
	//Flags_I[nRay] |= SHARP; /* Mark this ray "Sharp" */
	/* BIT_IS_ON(SHARP, iRay); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ERROR */
	SET_BIT(SHARP, iRay);
	...
	printf("PARA INTEGR: exp(-tau) = %g, dTb = %g, Tb = %g\n", 
	       exp(-OpDepth_I[iRay]), dTb, Intgrl_I[iRay]);
      }
	...
	/* GentleRay_I[iRay] = TRUE; // Mark this ray NOT sharp */
	//Flags_I[iRay] &= ~SHARP;  /* Mark this ray NOT sharp */
