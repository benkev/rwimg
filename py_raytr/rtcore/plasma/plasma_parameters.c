...
void plasma_parameters(int nRay, 
		       double Param_I[],
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
    ...
    Bfield_ID[iRay][0] = Msun_r3*(mr_3r2*x - prm->dirMx);
    Bfield_ID[iRay][1] = Msun_r3*(mr_3r2*y - prm->dirMy);
    Bfield_ID[iRay][2] = Msun_r3*(mr_3r2*z - prm->dirMz);
#endif
