...
class MainWindow:
    ...
    def make_implane(self):
        ...
        if(self.streamerSet!=None):
            for streamer in self.streamerSet:
                trace.make_streamer(streamer[0]*180/math.pi,
    ...
    def tbriv(self, widget, data=None):
        ...
        fig = figure()
        imshow(self.trace.tbriquv[:,:,1],cmap=cm.gist_heat)
        colorbar()
    ...
    def makeBatch(self, widget, data=None):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            f.write('import raytrace_cuda as rt\nimport numpy ' \
                    'as np\nimport os\n')
...
#makes the trace button
        ...
        self.graph_button_tbriv = gtk.Button("TbrIQUV Graph")
...
class StepWindow:
    ...
    def display(self):
        hold = np.empty((1,3))
        hold[0,:] = self.trace.pos[int(self.ray_x_spinner.get_value()),int(self.ray_y_spinner.get_value()),:]
    ...
    def graph(self):
        ...
        if(self.trace.mode>1):
            ...
            if(self.trace.mode==3):
                ...
                imshow(self.trace.tbriv[:,:,0],cmap=cm.gist_heat)
                colorbar()
                draw()
                ...
                imshow(self.trace.tbriv[:,:,3],cmap=cm.gist_heat)
                colorbar()
                draw()
        ...
        hold1[int(self.ray_x_spinner.get_value()),int(self.ray_y_spinner.get_value()),:] = hold1[lastX-1,lastY-1,:]
        points = np.resize(hold1,(np.size(self.trace.pos,0)*np.size(self.trace.pos,1),3))
        
        ax.scatter(points[:lastX*lastY-1,0],points[:lastX*lastY-1,1],points[:lastX*lastY-1,2],c='b')

        ax.scatter(hold2[:,0],hold2[:,1],hold2[:,2],c='r')
