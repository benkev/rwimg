 ...
 */
__device__ void Streamers_calculateDensity(Streamers_T oStreamers, double x,double y,double z, double r, double *Ne){
  ...
  //printf("Theta = %f, Phi = %f, Density=%f\n",oCurrentStreamer->pSpherical[1],oCurrentStreamer->pSpherical[2],oCurrentStreamer->dDensity);
 ...
 */
__device__ void Streamers_calculateBField(Streamers_T oStreamers, double x, double y, 
			       double z, double B[3]){

  double B0m;          // ~ field at the parabola vertex
  double B0s;          // ~ field in the stalk flux tubes
  double C2 = 1;            // Parabola vertex height    
  double yshm = 0.5;
  double yshs = 0.125;     // Distance of the stalk's B extremum from 0
  double ccy = 5;          // Y-compression coefficient
  ...
  /* rho, theta, phi */
  int num_streamers = 1;
  double spherical_coord_location[3];
  ...
  while(oCurrentStreamer!=NULL){
    ...
    mvmul(2, &rotation_matrix_1[0][0], hold1, hold2); 
    ...
    mvmul(2, &rotation_matrix_2[0][0], hold1, hold2);
    new_z = hold2[0];
    new_x = hold2[1];
    ...
    mvmul(2, &rotation_matrix_3[0][0], hold1, hold2);

    new_x = hold2[0]/oCurrentStreamer->pScale[0];
    new_y = hold2[1]/oCurrentStreamer->pScale[1];

    /* We want the streamer to be at the surface, not the center */
