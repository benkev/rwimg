 ...
 */
__global__ void setup_kernel(curandState *state, int nRay){
...
__global__ void second(Simulation *sim, int iIter) {
    ...
    //printf("before if(iRay>=sim->nRay) return; , iRay=%d\n", iRay);
    ...
    */
    ...
    if (BIT_IS_ON(PENETR, iRay)) { // Condition "Critical surface penetration"
        ...
        if (fabs(Eps) > sim->prm->TolEps) { // Another convergence step needed
            ...
            if (sim->DirPr_ID[iRay*3+2] < 0.0) { // if max count exceeded,
                SET_BIT(BADRAY|INACTIVE, iRay);  // Do not process ray anymore
            ...
            // DS = (DS_1 + DS_2)/2 
            sim->DS_I[iRay] = (sim->PosPr_ID[iRay*3+0] + 
			       sim->PosPr_ID[iRay*3+1])*0.5;
            ...
            //sim->DS_I[iRay] = sim->PosPr_ID[iRay*3+2];      
	    // Restore old DS !Not needed!
            HalfDS = 0.5*sim->DS_I[iRay];
		...
		if (sim->nTracedPts < 1) return;  // ===================== >>>
		/*
		 * Save positions of particular rays, 
		 * enumerated in int TracedPts_I[]:
		 *     Trajectories_I[iTracedPts,iIter,:] = Pos_ID;
		 */
		itrj = sim->idxTracedPts_I[iRay] + iIter*3;
    ...
    if (BIT_IS_OFF(SHARP, iRay)) { /* If the ray is NOT sharp */
        ...
        if (BIT_IS_ON(WASREFL, iRay)) {
            ...
            if (Curv >=  sim->prm->Tol2) { // If curvature is too high, 
		                           // decrease step by division:
                sim->DS_I[iRay] /= (2.0*sqrt(Curv/(sim->prm->Tol2)));
	...
	if (sim->nTracedPts < 1) return;  // ============================ >>>
	/*
	 * Save positions of particular rays, enumerated in int TracedPts_I[]:
	 *     Trajectories_I[iTracedPts,iIter,:] = Pos_ID;
	 */
	itrj = sim->idxTracedPts_I[iRay] + iIter*3;
...
void advance_beam(Simulation *sim_h, Simulation *sim_d) {
    ...
    num_blocks = nRays/num_of_threads + 1;

    // Sets up all the parameters needed for the plasma parameters array
    CUDA_CALL(cudaMalloc((void**)&devStates,sizeof(curandState)*nRays));

    setup_kernel<<<num_blocks,num_of_threads>>>(devStates, nRays);
    ...
    for(iIter=0;iIter<sim_h->nIter;iIter++){
        ...
        if(sim_h->scattering){
            scatter<<<num_blocks,num_of_threads>>>(sim_d,devStates);
            check_call("Scatter Call");
        }

        first<<<num_blocks,num_of_threads>>>(sim_d);
        check_call("Kernel First Call");

        intro(num_blocks,num_of_threads,sim_d,sim_h);
        check_call("Kernel Plasma Call");

	second<<<num_blocks,num_of_threads>>>(sim_d,iIter);
    ...
    printf("advance_beam(): after the loop\n");
