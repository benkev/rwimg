#
# Plot the Smerd solar brightness temperature profiles across the solar disk
...
for fname in flist:
    ...
    axx = axx[Ny/2:]
    axy = y0 + (y1-y0)*arange(Ny, dtype=float)/float(Ny-1) # Plot y-axix
    axy = axy[Ny/2:]
    figure(1);
