	       ...
	       double Freq, double OpDepth_I[nRay]) 
{
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
	...
	//
	// Use the Newton's method to reach the critical surface
	//
	...
	// DS = DS - Eps/dot_prod(grad(Eps),v)
	DS_I[iRay] = DS_I[iRay] - Eps/GradEpsDotDir;
       
	if (iRay ==  TrcRay) { 
	  printf("CONVERGENCE STEP CALC: DS = %g, Eps = %g\n", 
		 DS_I[iRay], Eps);
	  printf("CONVERGENCE STEP CALC: gradEpsDotDir = %g, gradEps =\n");
    ...
    if (Eps < 0.0) { // Eps < 0.0: Penetration!
      ...
      if (iRay ==  TrcRay) { 
	printf("PENETRATION OCCURRED: Eps = %g\n", Eps);
      }
