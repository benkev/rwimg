...
void beam_path(void (* Get_Plasma_Density)(), int nRay, 
	       ...
	       short *NewEntry, 
	       double Freq, double OpDepth_I[nRay]) 
{
  //
  ...
  // refractive index, which is the square root of the diellectric permittivity
  ...
  int const static TrcRay = 22700; //25100;
  double const static TolEps = 1e-6;
  short const static CntMax = 50;
  // Bits for Flags_I
  short const static EpsLT0 = 0x0001;  // Eps < 0; Critical surface search
  ...
  double static *gev_I = 0;               // [nRay], GradEpsDotDir old
  ...
  if (*NewEntry) {
    *NewEntry = FALSE;
    ...
    gev_I = (double *)calloc(nRay, sizeof(double));
  ...
  for (iRay = 0; iRay < nRay; iRay++) {
	...
	SolDist += pow(Pos_DI[i][iRay],2);
      ...
      printf("AFTER Get_Pl_Den: SolDist = %g\n", SolDist);
    ...
    if (Flags_I[iRay]&EpsLT0) { // Condition "Critical surface penetration"
      //if (Flags_I[iRay] == TRUE) {


      if (iRay ==  TrcRay) { 
	  printf("FLAGS:PENETRATION: [%i,%i]\n", iRay/200, iRay%200);
      }
	...
	//Flags_I[iRay] = FALSE;
	Flags_I[iRay] &= ~EpsLT0;  // Leave the penetration condition
	Flags_I[iRay] |= WasRefl;  //

	//DS_I[iRay] = PosPr_DI[n2Ry+iRay];      // Restore old DS
	HalfDS = cHalf*DS_I[iRay];
	...
	// The tau is integral along ray path of the absorption coefficient 
	// kappa
	//
	//// Here Eps is close to zero, |Eps| < TolEps, and a minus sign
	//// means nothing, but it can cause NaN while sqrt(Eps) 
	//if (Eps < 0.0) Eps = 0.0;  
	...
	//CAbsorp = xi*pow(Ne,cTwo)/(pow(Te,1.5)*pow(Freq,cTwo)*sqrt(Eps));
	//dOpDepth = CAbsorp*cTwo*DS_I[iRay]*cSolarRadius;
	...
	//	 Pos_DI[2][iRay]);
	//  printf("REFLECTION: v = (CHANGED)");
	//  printf("%g %g %g\n", Dir_DI[0][iRay], Dir_DI[1][iRay], 
	//	 Dir_DI[2][iRay]);
	//  printf("REFL INTEGR: Te = %g, Ne = %g\n", Te, Ne);
	//  printf("REFL INTEGR: CAbsorp = %g, dOpDepth = %g, OpDepth_I =%g\n", 
	//	 CAbsorp, dOpDepth, OpDepth_I[iRay]);
	//  printf("REFL INTEGR: exp(-tau) = %g, dTb = %g, Tb = %g\n", 
    ...
    if (Eps < 0.0) { // Eps < 0.0: Penetration!
      ...
      Flags_I[iRay] |= EpsLT0;  // Mark "penetration"
      ...
      // left and right values of DS and Eps
      //
      PosPr_DI[iRay] = 0.0;                  // Store here DS_1
      PosPr_DI[nRay+iRay] = cTwo*DS_I[iRay]; // Store here DS_2
      PosPr_DI[n2Ry+iRay] = DS_I[iRay];      // Store here old DS
      DirPr_DI[iRay] = 1.0;                  // Store here Eps_1
      DirPr_DI[nRay+iRay] = -1.0;            // Store here Eps_2
	...
	Flags_I[iRay] &= ~WasRefl;  // Leave the reflection condition
	...
	//
	// Ensure DS is small enough to correctly predict Eps one step ahead
	//
	if (GradEpsDotDir/gev_I[iRay] > cTwo) {
	  DS_I[iRay] *= cHalf;
	  // r = r_hb
	  Pos_DI[0][iRay] = PosHalfBk_D[0];
	  Pos_DI[1][iRay] = PosHalfBk_D[1];
	...
	//DSEpsPrec = fabs(cTwo*EpsPrec/GradEpsDotDir);
	//if (DS_I[iRay] > DSEpsPrec) {// Testing Eps precision
	//  //Te = (SolDist > r_chromo) ? Te_corona : Te_chromo;
	//  DS_I[iRay] = (DSEpsPrec > AbsMinStep) ? DSEpsPrec : AbsMinStep;
	//  // r = r_hb
	//  Pos_DI[0][iRay] = PosHalfBk_D[0];
	//  Pos_DI[1][iRay] = PosHalfBk_D[1];
	//  Pos_DI[2][iRay] = PosHalfBk_D[2];
    ...
    if (!(Flags_I[iRay]&EpsLT0)) { // i.e. if not "penetration"
      ...
      // Save GradEpsDotDir in gev_I[]
      gev_I[iRay] = GradEpsDotDir;
    }
