...
class implane():
    ...
    def __init__(self, grid=(10,10), rect=(-5., -5, 5., 5.),
                 ...
                 mode='Tbr', cnu=3.0, msun=1.0, traj=False, units='K',
                 scattering=False):
        """
        ...
        if mode == 'basic' or mode == 1:
            self.mode = pl.int32(1)     # No Tbr or TbrIQUV calculation;
        
            self.traj = True  # Store ray trajectories
        elif mode == 'Tbr' or mode == 2:
            self.mode = pl.int32(2)     # Tbr calculation;
        
        elif mode == 'TbrIQUV' or mode == 3:
            self.mode = pl.int32(3)     # No Tbr or TbrIQUV calculation;
        ...
        self.TRACED =    0x0020   # The ray is traced
        ...
        if self.mode == 3:
            self.tbriquv = pl.empty((grid[1], grid[0], 4), dtype=pl.double)
            self.tbriquv[:] = 0.0
        else:
            self.tbriquv = pl.empty(0)
        ...
        if(scattering==False):
            self.scattering = pl.int32(0)
        else:
            self.scattering = pl.int32(1)
        ...
        # Array of coordinate triplets, trajectories[nTracedPts,niter,3].
        # It is created if the integer array tracePoints[nTracedPts,2] of
        # image plane coordinates of the rays whose trajectories must be
        # saved is passed in the call to self.trace(). 
        self.trajectories = pl.empty(0, dtype=float)

        # Array of integer coordinates within the image plane,
        # tracePoints[nTracedPts,2], of the rays 
        # whose trajectories to be recorded in self.trajectories array.
        # It is created after the namesake parameter, tracePoints, is
        # passed at the call to self.trace() method.  
        self.tracePoints = pl.empty(0, dtype=pl.int32)

        # The 1D array of integer coordinates in the image plane treat
        self.passedPoints = pl.empty(0, dtype=pl.int32)

        ## # Thw 1D array containing numbers of the traced rays whose
        ## # trajectories
        ## # are stored in trajectories[nTracedPts,niter,3]. The location in
        ## # this table (found through bisection search for a ray number)
        ## # is the index into the int ptraj[nTracedPts] array, containing
        ## # current pointers to the coordinat xyz-triplets in trajectories[].
        ## self.itray = pl.empty(0, dtype=pl.int32)
        
        # Maybe, in some time I will understand the wisdom of introducing
        # this array. However, right now it is beyond my comprehension...
        self.lastStep = pl.empty(0, dtype=pl.int32)
    ...
    def trace(self, niter=None, tracePoints=None):
        """
        ...
        parameter: tracePoints-A 2D array of integer [i,j] grid coordinates
        on the implane to trace throughout the algorithm.

        returns: if tracePoints was not None, then the trajectories of all
        ...
        if niter is None:
            ...
            niter = pl.int32(niter)

        if tracePoints is not None:
            self.tracePoints = pl.array(tracePoints, dtype=pl.int32)
            # Turn 2D table tracePoints[:,2] into a linear array passedPoints
            self.ntracedpts = len(tracePoints[:,0])
            self.passedPoints = pl.empty((self.ntracedpts), \
                                         dtype=pl.int32)        
            for i in range(self.ntracedpts):
                iy = tracePoints[i,1]
                ix = tracePoints[i,0]
                self.passedPoints[i] = iy*self.grid[0] + ix
                self.flags[iy,ix] |= self.TRACED   # Flag the traced rays
###             self.itray[i] = 

            self.trajectories = pl.empty((self.ntracedpts,niter,3), \
                                         dtype=pl.double)
            self.lastStep = pl.empty(self.ntracedpts, dtype=pl.int32)
            self.lastStep[:] = 0
            
        self.theta.append(float('nan'))
        theta = pl.empty((len(self.theta)),dtype=pl.double)
        theta[:] = self.theta[:]
        self.phi.append(float('nan'))
        phi = pl.empty((len(self.phi)),dtype=pl.double)
        phi[:] = self.phi[:]
        self.orientation.append(float('nan'))
        orientation = pl.empty((len(self.orientation)),dtype=pl.double)
        orientation[:] = self.orientation[:]
        self.density.append(float('nan'))
        density = pl.empty((len(self.density)),dtype=pl.double)
        density[:] = self.density[:]
        self.baseStrength.append(float('nan'))
        baseStrength = pl.empty((len(self.baseStrength)),dtype=pl.double)
        baseStrength[:] = self.baseStrength[:]
        self.stalkStrength.append(float('nan'))
        stalkStrength = pl.empty((len(self.stalkStrength)),dtype=pl.double)
        stalkStrength[:] = self.stalkStrength[:]
        self.scale.append(float('nan'))
        scale = pl.empty((len(self.scale)),dtype=pl.double)
        scale[:] = self.scale[:]
        ...
        if(self.units == 'Jy'):
            ...
            alpha = (self.rect[2]-self.rect[0])/self.grid[0]/dist
            beta = (self.rect[3]-self.rect[1])/self.grid[1]/dist
...
def run_freq_range(file,
    ...
    alpha = (rect[2]-rect[0])/grid[0]/dist
    beta = (rect[3]-rect[1])/grid[1]/dist
    solid_angle = alpha*beta
    ...
    for j in [0]:
        ...
        prihdr.update('NAXIS1', pl.size(data,2), 'X pixels')
        prihdr.update('NAXIS2', pl.size(data,3) , 'Y pixels')
        ...
        prihdr.update('CDELT1', (rect[2]-rect[0])/float(pl.size(data,2))/dist*180/np.pi, 'Degrees/Pixel')
        ...
        prihdr.update('CDELT2', (rect[3]-rect[1])/float(pl.size(data,3))/dist*180/np.pi, 'Degrees/Pixel')
