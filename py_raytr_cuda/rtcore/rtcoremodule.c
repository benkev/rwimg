...
#define OBJ_TO_INOUT_ARRAY_DOUBLE(obj,arr)     \
	if (obj == NULL) return NULL;	        \
arr = (PyArrayObject *) PyArray_FROM_OTF(obj, NPY_DOUBLE, NPY_INOUT_ARRAY); \
if (arr == NULL) return NULL;

#define OBJ_TO_INOUT_ARRAY_INT(obj,arr)     \
	if (obj == NULL) return NULL;	        \
...
PyObject *trace_beam(PyObject *self, PyObject *args){
	...
	double *Theta_I = NULL, *Phi_I = NULL, *Orientation_I = NULL, *Density_I = NULL;
	...
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjParam_I,        pyarParam_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjPos_ID,         pyarPos_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDir_ID,         pyarDir_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDS_I,           pyarDS_I);
	if (rtmode == 2)    /* Brightness temperature */
		OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjTbr_I,        pyarTbr_I);
	if (rtmode == 3) {/* Brightness temperature in Stokes param. I,Q,U,V */
		OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjTbrIQUV_IP,   pyarTbrIQUV_IP);
	}
	if (rtmode == 2 || rtmode == 3)
		OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjOpDepth_I,    pyarOpDepth_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjRho_I,          pyarRho_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjGradRho_ID,     pyarGradRho_ID);
	if (rtmode == 3)    /* Magnetic field for Tb in Stokes param. I,Q,U,V */
		OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjBfield_ID,      pyarBfield_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjPosPr_ID,       pyarPosPr_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDirPr_ID,       pyarDirPr_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDS_New_I,       pyarDS_New_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDistToCrSurf_I, pyarDistToCrSurf_I);

	OBJ_TO_INOUT_ARRAY_INT(pyobjTracedPts_I,       pyarTracedPts_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjTrajectories_I, pyarTrajectories_I);
	OBJ_TO_INOUT_ARRAY_INT(pyobjLastStep_I,        pyarLastStep_I);

	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjTheta_I, pyarTheta_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjPhi_I, pyarPhi_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjOrientation_I, pyarOrientation_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDensity_I, pyarDensity_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjBaseStrength_I, pyarBaseStrength_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjStalkStrength_I, pyarStalkStrength_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjScale_I, pyarScale_I);

	/* The only non-double and non-int array: individual treatment */
	...
	CHKSIZE_AND_GETPTR(pyarTrajectories_I, nTracedPts*3*nIter, double,   \
			   Trajectories_I);
	...
	prm = (struct param *) Param_I; /* Access parameters by structure field names */
...
PyObject *raydir(PyObject *self, PyObject *args) {
	...
	/* Convert Python objects to Numpy array objects */
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjObs_D,    pyarObs_D);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjXruler_I, pyarXruler_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjYruler_I, pyarYruler_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDir_ID,   pyarDir_ID);
...
PyObject *raysph(PyObject *self, PyObject *args) {
	...
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjObs_D,  pyarObs_D);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjDir_ID, pyarDir_ID);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjIsec_I, pyarIsec_I);
	OBJ_TO_INOUT_ARRAY_DOUBLE(pyobjPos_ID, pyarPos_ID);
